...
inline void inverse33(
    Eigen::Ref<Eigen::Matrix3d> result,
    Eigen::Ref<const Eigen::Matrix3d> A)
{
    double determinant =
    ...
    result(2, 1) = -(A(0, 0) * A(1, 2) - A(0, 2) * A(1, 0)) * invdet;
    result(0, 2) = (A(1, 0) * A(2, 1) - A(1, 1) * A(2, 0)) * invdet;
    result(1, 2) = -(A(0, 0) * A(2, 1) - A(0, 1) * A(2, 0)) * invdet;
