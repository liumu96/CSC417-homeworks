...
igl::opengl::glfw::imgui::ImGuiMenu &Visualize::viewer_menu()
{
    return menu;
}
    ...
    ImU32 plot_col)
{
    ...
    { return ImVec2((pix.x - bb.Min.x) / Canvas.x, (pix.y - bb.Min.y) / Canvas.y); };
    auto normalized_to_pix = [&bb, &Canvas](ImVec2 norm)
    { return ImVec2(norm.x * Canvas.x + bb.Min.x, Canvas.y - norm.y * Canvas.y + bb.Min.y); };
    auto data_to_normalized = [&](ImVec2 state)
    { return ImVec2((state.x - g_energy[0][0]) / (T_bounds.y), (state.y - E_bounds.x) / (E_bounds.y - E_bounds.x)); };
...
void Visualize::add_object_to_scene(
    ...
    const Eigen::MatrixXd &V_skin,
    const Eigen::MatrixXi &F_skin,
...
bool Visualize::mouse_down(
    igl::opengl::glfw::Viewer &viewer, int x, int y)
{
    g_mouse_win = Eigen::Vector3d(
        g_viewer.current_mouse_x,
        viewer.core().viewport(3) - g_viewer.current_mouse_y, 0.);
    igl::unproject(
        g_mouse_win,
        g_viewer.core().view,
        g_viewer.core().proj,
...
bool Visualize::mouse_move(
    igl::opengl::glfw::Viewer &viewer, int x, int y)
{
    g_mouse_drag = Eigen::Vector3d(
                       g_viewer.current_mouse_x,
                       viewer.core().viewport(3) - g_viewer.current_mouse_y, 0.) -
                   g_mouse_win;
    g_mouse_win = Eigen::Vector3d(
