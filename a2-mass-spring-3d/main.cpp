...
Eigen::MatrixXi E;  // edges of simulaition mesh (which will become springs)
Eigen::MatrixXd l0; // original length of all edges in the mesh
...
int main(int argc, char **argv)
{
    ...
    if (M.rows() == 0)
    {
        std::cout << "mass_matrix_particles not implemented ... exiting \n";
        exit(0);
    }
    ...
    // vector x0 contains position of all fixed nodes, zero for everything else
    ...
    Visualize::viewer().launch_init(false, "Mass-Spring Systems", 0, 0);
    Visualize::viewer().launch_rendering(true);
    simulating = false;
    Visualize::viewer().launch_shut();

    std::cout << "mass-spring-3d" << std::endl;
    return 0;
}
                         ...
                         Eigen::Ref<const Eigen::VectorXd> qdot)
        {
            ...
            for (unsigned int pickedi = 0; pickedi < Visualize::picked_vertices().size(); pickedi++)
            {
                ...
                dV_spring_particle_particle_dq(
                    dV_mouse,
                    mouse,
                    (P.transpose() * q + x0).segment<3>(3 * Visualize::picked_vertices()[pickedi]),
                    0.0,
